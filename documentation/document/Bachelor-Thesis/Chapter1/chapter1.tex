...
The signal W($\phi$) shows us , what happen if calculations of proposition $\phi$ need some time, in that case $y$=3 clock cycles.
As I mentioned, W($\phi$) is like signal $\phi$ shifted to the right.
...
The most important think here is that the observer are working delayed. Every new observer stage starts his work after the observer stage before.
This is important, because every clock cycle must be covered as long as the calculation of W($\phi$) needs. 
...
It starts with the question of how many observer stages do we actually need ? We already know that at least y are necesseray.
If we have to define a number of observer stages with variable ''m'' than following condition mus be hold: $m \ge y$.
Depending on how long the evaluation of a proposition needs, a minimum of y observer stages are necessary, but upwards 
...
used for every arbitrarily choose of $\tau$. In \cite{RTFMBJ13} this setting is fixed.
My implementation of an invariant observer is able to change that parameter during run-time, but this feature is not 
specified in the requirements, and should be treated as so.\newline
